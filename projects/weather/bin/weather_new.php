...
<?
    ...
    if($xml = @simplexml_load_file($xmlfile)) {
      ...
      if($city=="Phoenix") {
        ...
        if($weather_icons[$index]!="") {
          ...
          $iconsrc="/weather/imgs/icons/".$weather_icons[$index].".png";
        } else {
  ...
  if($error==0) {
    ...
    $filename = '/apps/generated/generated/phxweathericonsrc.txt';
    if ($iconsrc !== @file_get_contents($filename)) {
      if (false === file_put_contents($filename,$iconsrc)) {
        // problem
      } else {
        log_for_sync($filename);
      }
...
function log_for_sync($filename) {
  ...
  if ($loghandle = fopen("/www/enigma/pubpush/$now.log",'a')) {
    if (flock($loghandle,LOCK_EX)) { // do an exclusive lock
      ...
      flock($loghandle,LOCK_UN); // release the lock
    }
    fclose($loghandle);
  }
}
